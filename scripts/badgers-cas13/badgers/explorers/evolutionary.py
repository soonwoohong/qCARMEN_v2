...
import flexs
from ..utils import prepare_sequences as prep_seqs
from ..utils.prepare_sequences import track_performance as track_performance
import numpy as np
    ...
    def propose_sequences(self, measured_guides: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]: 
        ...
        while total_guides_evaluated < self.model_queries_per_batch:
            ...
            if(len(child_guides_to_evaluate) > 0):
                ...
                new_rows = pd.DataFrame({"guide": child_guides_to_evaluate, "fitness": children_fitness})
                guides_df = pd.concat([guides_df, new_rows], ignore_index = True)
        ...
        if(self.local_s):
            ...
            guides_df = pd.concat([guides_df,pd.DataFrame({"guide": mut_seqs,"fitness": mut_perf})], ignore_index = True)
