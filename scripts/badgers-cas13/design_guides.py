...
def compile_results_across_sites(explorer_name, args):
    ...
    # updated:
    # replace inefficient pandas append with list append
    ...
    all_results = []
    for file in results_files:
        all_results.append(pd.read_csv(results_dir + file, sep = '\t').iloc[0:args.n_top_guides_per_site])

    all_results_df = pd.concat(all_results, ignore_index=True) if all_results else pd.DataFrame(columns = ['algo',
                                                                                                           'start_pos',
                                                                                                           'guide_sequence',
                                                                                                           'fitness',
                                                                                                           'perc_highly_active',
    ...
    if(args.objective == 'multi'):
        ...
        target_set_results_df = all_results_df.sort_values(by = 'fitness', ascending=False).reset_index(drop = True).iloc[0:args.n_top_guides*2]
...
if __name__ == '__main__':
    ...
    # added:
    # merge them together and save the result into one single file
    ...
    final_output_file = os.path.join(args.results_path, 'final_results.tsv')
    final_output_data = []

    for explorer in exploration_algos: 
        compile_results_across_sites(explorer, args)
        final_output_data.append(pd.read_csv(os.path.join(args.results_path, f'results_compiled_{explorer}.tsv'), sep = '\t'))

    final_output_df = pd.concat(final_output_data, ignore_index=True).drop_duplicates(subset=['guide_sequence'], ignore_index=True).sort_values(by = 'fitness', ascending=False)
    final_output_df.iloc[0:args.n_top_guides].to_csv(final_output_file, sep = '\t', index=False)
